...
try {
    ...
    $run = function () use ($directory): string {
        $command = 'FINTS_OFFLINE_TEST=1 DATA_PATH=' . escapeshellarg($directory) . ' '
    ...
    echo "Safe cron regression tests passed (PayPal-only, mixed, disabled accounts; offline).\n";
} finally {
    unset($db);
