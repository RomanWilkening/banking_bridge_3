        ...
        foreach ($array as $date => $statement) {
            if ($result->hasStatementForDate($date)) {
                ...
                if (isset($statement['start_balance']['amount'])) {
                    ...
                    $statementModel->setStartBalance($startAmount);
                }
                if (isset($statement['end_balance'])) {
                    // The MT940 parser already negates debit amounts, so we use the amount directly
                    // without applying the credit/debit multiplier again (which would cause double negation)
